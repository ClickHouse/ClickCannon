...
- **Metrics support (ingestion)** — A new `metrics` data type joins `logs`, `traces`, and `profiles` across the disk, generate, and insert pipelines. Set `app.data_type: metrics` plus `app.metrics_type` (`gauge`, `sum`, `histogram`, `exponential_histogram`, or `summary`) to target one of the five OTel exporter metrics table schemas per run. Generate mode produces whole series (points sharing metric name, service, and attributes with advancing timestamps, shaped via `generate.metrics`), sums are cumulative counters, and histogram counts/sums/min/max are internally consistent; exemplar columns are emitted empty. Disk mode replays pre-exported `.native` files (`disk.metrics_<type>_path`), and insert targets `insert.clickhouse.metrics_<type>_table` (defaulting to the exporter table names).
- **Metrics support (OTLP export)** — The otel export sink now converts metrics blocks into OTLP `ExportMetricsServiceRequest`s and sends them over gRPC, enabling collector throughput benchmarking in data points per minute (DPM = `generate.rows_per_second` × 60). Data points are grouped into Metrics by (name, unit, description) under their resource/scope; sums carry temporality and monotonicity; histogram, exponential histogram, and summary shapes pass through as generated (exemplars are not exported). See the new "Benchmarking an OTel Collector with Metrics" README section for batch-sizing guidance and ceiling-finding methodology.
- **OTLP export latency metric** — New `otel_export_latency_micros` sample metric records the duration of each successful export flush (attributes: `worker_id`, `rows`). Rising flush latency is the earliest signal that the target collector is saturating, before exports start failing.
